...
function ProductDescription({ description, prductID }) {
    ...
    async function fetchReview() {
        let response = await fetch(`http://localhost:5000/api/fetchReview/${prductID}`)
        response = await response.json()
        setAllFetchedReview(response.fetchedReview)
        const filteredReview = response.fetchedReview.filter(item => item.reviewUserId === userID);
                                        ...
                                        filterReview.map((item) => {
                                            return (<>
                                                <div className="d-flex gap-2 align-items-center">
                                                    ...
                                                    <p className='mb-0'><strong>{item.review}</strong></p>
                                                    <p className='mb-0'><strong>{item.rating}</strong></p>
                                                    <i onClick={() => handleEditReview({ review: item.review, rating: item.rating })} class="fa-solid fa-pen-to-square"></i>
                                                </div>
                                                <div>
                                                    {
                                                        editReview && <>
                                ...
                                <Row>
                                    ...
                                    <Col>
                                        ...
                                        <ReactStars
                                            ...
                                            value={rating}
                                        />
                                        <button className='checkout' onClick={handleSubmitReview}>Submit review</button>
                                    </Col>
                                </Row>
                        }
                        <Row>
                            {
                                allFetchedReview && allFetchedReview.map((item, index) => {
                                    return (
                                        <>
                                            <div className='border-bottom py-2'>
                                                <p className='mb-0'>{index + 1}. {item.review}</p>
